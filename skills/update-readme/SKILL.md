...
- Depois do setup, quando o README ainda descreve o template em vez do
  projeto.
...
| `pyproject.toml` (scripts)  | Entradas da CLI e da GUI                   |
...
| `.github/workflows`, `.pre-commit-config.yaml` | Verificação automática  |
...
9. Comandos (tabela).
10. Build para diferentes ambientes, quando o projeto gera executável.
11. Estrutura resumida (árvore comentada).
12. Como usar agentes.
13. Como criar uma feature.
14. Como registrar uma decisão.
15. Variáveis de ambiente (quando existir `.env.example`).
...
- Comando portável: mostre a forma `python scripts/dev.py <tarefa>`, que
  funciona nos três sistemas, e cite o atalho do `make` como conveniência.
- Quando o comportamento diferir entre Windows, macOS e Linux, diga qual
  é a diferença.
...
3. Remova o que só valia para o template — "Criar um novo projeto a
   ...
   `python-project-template` — quando o projeto já passou pelo setup.
4. Escreva as seções na ordem padrão.
5. Cite somente tarefa que existe no registro `TASKS` de `scripts/dev.py`
   e link que aponta para arquivo existente:
   `python scripts/dev.py check-docs` reprova o contrário.
6. Registre as limitações reais do projeto. Não prometa o que não existe.
7. Rode `python scripts/dev.py check-docs`.
