...
description: Registra em docs/entregas a evidência do que foi entregue — funcionalidades, testes e validações executados; use depois de implementar um plano com implement-feature e com o validate verde.
---
...
- Enquanto `python scripts/dev.py validate` não estiver verde: sem
  validação não há evidência a registrar.

## Processo

1. Confirme que a implementação terminou e que
   `python scripts/dev.py validate` passou. Se algo estiver falhando,
   pare e corrija antes de documentar.
...
4. Liste os testes adicionados ou atualizados e o que cada um cobre.
   Registre a cobertura obtida, quando a entrega mexeu nela.
...
6. Descreva cada funcionalidade pelo que passou a ser possível para quem
   usa — subcomando da CLI, ação na GUI —, não pela implementação.
7. Registre o que ficou fora do escopo, as limitações e as pendências
   conhecidas, incluindo diferenças de comportamento entre sistemas
   operacionais.
...
11. Execute `python scripts/dev.py check-docs` para garantir que os links
    e os comandos citados existem.
